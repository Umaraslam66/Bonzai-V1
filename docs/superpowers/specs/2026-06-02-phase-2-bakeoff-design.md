...
- **The regime-transfer / contract-not-read catches are a meta-pattern, not one-offs** — a number/method valid in one regime (or a contract assumed rather than read) does not transfer: (1) NLL exact-vs-ELBO across families; (2) the emergence-confound (probe under-trained 190×); (3) the resolution unit (per-cell 0.076 ≠ per-feature); (4) 6ND across backbones; (5) the budget magnitude (80×→7.6×); (6) **the building-polygon contract** — the sealed decoder returns building closed rings as `LineString` BY CONTRACT (`decoder.py:145-157`, consumer must promote), the scaffold metric filtered `type=="Polygon"` and never promoted, so `n_polygons` silently read 0 on real data (the *dominant* cause of the probe's `n_polygons=0`, not under-training). Caught in execution by the small-before-big real-data density check + the §6 `0.0` anti-signal, BEFORE the diagnostic measured a vacuous floor. Fix = Task 1.5 `promote_building_rings` (§9 construction-identity keying). **Audit every inherited number AND every consumed contract: "what regime was it measured in? did I read the upstream contract or infer it?"**
