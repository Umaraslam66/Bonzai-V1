...
**Manifest schema (v1):**
- `sampler_schema_version`, `release` (`2026-04-15.0`), `floor_sha256` (the `95abb88` floor it targets — lineage lock), **`census_sha256`** (sha256 of the input census parquet — pins the EXACT cell pool unambiguously). **PI ratification 2026-06-22:** `census_sha256` is chosen OVER a `source_holdout_manifest` ref because the 1,952-tile pool is **4 per-city holdout manifests**, an ambiguous ref to bake into a sha-locked artifact; the census parquet is the single materialized pool and its byte-determinism (§9 test) makes its sha a clean, reproducible lineage anchor.
...
- `held_out_cities`.
- `strata[]`: one row per targeted `(city, 4-tuple)` — `owed_metrics` (both | road_only), `binding_metric`, `floor_n_binding`, `available_cells`, `real_fpc_binding`, `n_cells_target`, `n_cells_selected`, `ceiling_bound: bool`.
...
**Schema reconciliation with the committed Task-6 `build_manifest` (2026-06-22):**
- `census_sha256` is **NOT YET emitted** by the committed `build_manifest`/CLI — wiring it is a small, gated **pre-build code step** (CLI computes `compute_sha256(census_parquet_bytes)`, passes it to `build_manifest`, which records it; add a test). Land it before the first manifest is sealed — no artifact exists yet, so nothing is invalidated. (Docs-only ratification now; code at PI word + Leonardo redeploy.)
- `expected_gen_features[metric]` and per-stratum/per-city **totals were intentionally dropped** — superseded by the emitted `n_cells_selected` + `floor_n_binding` + `real_fpc_binding` (the optimistic gen ceiling is derivable as `n_cells_selected·real_fpc_binding`). Schema above matches the committed Task-6 output (plus the pending `census_sha256`).
- **Naming-seam note (for the gated generation-loop author, no change now):** the manifest's `cells[].density_bucket` (conditioning input) maps to `DecodedCell.cell_density_bucket` (`gen_realism.py:51`, generation output) — different objects at different pipeline stages; the loop wiring carries the value across. Flagged so it isn't a footgun.
...
- **R3 — RESOLVED: ceiling-bound set is SMALL (read-only census, 2026-06-21, from floor `95abb88` `n_a`/`n_b`).** The optimistic ceiling at `gen_ratio=1`, full draw, equals the real building-feature count `n` (= `available_cells × (n/available_cells)`), which = the floor's qualify quantity. Flooring required `n ≥ 50`, so **0/119 building_area-floored strata are ceiling-bound by construction** (min observed `n = 59`; the thin-CELL census does NOT translate because thin-cell building-floored strata are building-dense). `n` is large for nearly all strata (median 1170, p25 306, max 42735), so gen_ratio tolerance is generous (the median stratum still reaches 50 at `gen_ratio` 0.043). Ceiling-bound rises slowly with headroom: **0 @1.0, 2 @1.25, 3 @1.5, 6 @2.0, 17 @3.0** — and is **glasgow-concentrated** (glasgow min `n`=59, 4 strata `n`<100; krakow 0; munich 1; eisenh 1). **#21 demotion risk is negligible:** `decide()` computes `n_reference` over real features on floored strata *independent of gen-side exclusions* (`bakeoff_decision.py:526-533`), so a ceiling-bound exclusion does NOT shrink `n_reference` or raise the `C/√n_ref` floor — its only effect is dropping ≤~4 of a city's ~70+ scored strata from the per-city median (robust). **No city is at `SECOND_REGION` risk at any headroom ≤ 2.** Residual: this is the `gen_ratio=1` lower bound; the live watch-list is the ~6 low-`n` glasgow strata whose fate depends on the *measured* `gen_ratio` at first generation — already routed via exclude-and-report + the Gate-5 consumer check. **Verdict: SMALL → headroom/`target_features` do not need revisiting before build.**
...
- **R5 — §9 coverage gap: a ceiling-bound stratum's NON-binding metric short is a data limit, not a sampler bug (PI-logged 2026-06-22, NO code change).** The Gate-5 §9 split keys exclude-and-report on `metric == binding_metric AND ceiling_bound`; the `else` (fail-loud) branch therefore *also* catches a ceiling-bound stratum whose **non-binding** metric (road in a "both" stratum) falls short — yet that is the SAME data limit (the cell pool was exhausted; you cannot add cells you already took), so the fail-loud "sampler under-sized it" verdict would be wrong there. **UNREACHABLE under R3** at headroom ≤ 2.0 (0/119 building-floored strata are ceiling-bound; road-only strata have road as the binding metric, so the case cannot arise) — hence fail-loud on an unreachable path is correct-for-now and beats a speculative broadened exclusion that could mask a genuine model pathology (roads catastrophically under-emitted despite abundant cells). **ACTIVATION CONDITION — revisit before raising `headroom` > 2.0** (or applying to a corpus where R3's zero-ceiling-bound guarantee does not hold): the PI then decides whether to widen the ceiling exclusion to any-metric-when-ceiling or keep it loud as a pathology signal. The latent branch carries this note in code (`verify_gen_coverage`, commit `5e80f1d`). No speculative handling added.
