...
| Structural sentinels (on-disk) | **2** | `<feature>` (id 509), `<feature_end>` (id 510). Documentation correction at T8 plan-write (2026-05-28): the original §2.2 wording named six structural sentinels, but `<anchor_x>` / `<anchor_y>` / `<direction>` marker / `<magnitude>` marker are positional descriptors of value-token classes, NOT separate on-disk sentinels — the grammar is self-delimiting via token-class ID ranges + positional convention + strict (dir, mag) alternation (verified at T8 plan-write self-delimiting check). The 4 marker names had no allocated IDs anywhere in the locked inventory because they were never real on-disk tokens. The 2 real sentinels (`<feature>`, `<feature_end>`) were allocated append-safely by consuming IDs 509, 510 from the front of BP2 `reserved_v2_headroom` (now 511–1499). Family classification: `structural` (grammar primitives), distinct from the `encoding_primitive` family of the anchor/direction/magnitude sub-blocks whose ID neighborhood they share. `<cell_start>` / `<cell_end>` dropped per BP2 → BP4 revision (§13.1); cell boundaries via parquet row structure. See §13.1 Halt-2-cascade entry for the gap-discovery + resolution audit trail. |
...
| Round tie-breaking | **LOCKED at Halt 5 (2026-05-28):** Python `round()` round-half-to-even (PEP 3141). Coordinate quantization is `int(round(coord_m / quantum))`. Rationale: (a) determinism requires one rule pinned, (b) banker's is bias-free for coordinate snapping (no DC offset on symmetric input distributions). | Verification at Task 5a confirmed `round()` is banker's in this env (6/6 constructed boundary cases pass; see `reports/sub_f_task_5a_rounding.yaml`). Sub-D `int(round(...))` usage at `frequency_analysis.py` percentile-index + `evidence.py` percentile-index is noted as house-style consistency, NOT load-bearing precedent (those sites quantize indices, not coordinates). Adjacent: sub-C `coords.py` uses `math.floor` for cell-binning; sub-C + sub-F's distinction (floor bins, round snaps) is correct-by-operation, not a latent inconsistency. No §9.6.1 cascade. |
...
| OSM feature iteration order | **LOCKED at Halt 5 (2026-05-28):** sub-C row order from `features.parquet`. Concrete sort key: `(cell_i, cell_j, feature_class, source_feature_id)`. Sub-F inherits this iteration order without re-sorting. | `src/cfm/data/sub_c/io.py` `write_features_parquet` lambda (lines `:219-221` at audit time; cite via function + snippet per `feedback_content_anchored_cites`). |
| Vertex iteration within feature | **LOCKED at Halt 5 (2026-05-28) as CANONICALIZE for the 3 pure-redundancy DOFs only** (closed-ring start vertex, closed-ring winding, multi-part order with tiebreak) per `feedback_ambiguous_third_branch_in_verification` defend default. The Task 5a script verified only the sub-C ↔ parquet hop (1 of 3 chain hops); Overture → sub-A DuckDB httpfs ordering is undocumented. **Open-polyline direction (LineString and closed-LineString such as roundabouts) is preserved, NOT canonicalized** — BP1 vocab grep (2026-05-28) confirmed no direction-encoding tokens (`oneway=*`, `waterway` flow, `coastline` land-side, `cycleway:` direction), so canonicalizing direction would silently destroy recoverable OSM semantics (highway-motorway oneway-by-default; coastline land-on-right; river-downstream). Canonical form specified in §5.6; encoder implements idempotently. | Task 5a evidence: `reports/2026-05-23-phase-1-sub-F-task-5a-halt.md` + `reports/sub_f_task_5a_vertex_order.yaml`. BP1 direction-semantics grep evidence: §5.6 "Open-polyline direction preservation" note. |
...
### 5.6 Vertex-order canonical form (LOCKED at Halt 5)

**Halt 5 outcome (2026-05-28):** CANONICALIZE. The Task 5a script verified only the sub-C ↔ parquet round-trip hop (20/20 exact match), but the Overture → sub-A hop is not documented as vertex-order-preserving — effective outcome per `feedback_ambiguous_third_branch_in_verification` is (c) ambiguous-but-locally-stable, and the defend default applies. The 1-of-3-hops scope is the same defect shape the script's verification was meant to catch; defending against it eliminates the entire defect class rather than narrowly checking one hop.

**Canonical form (sub-F encoder applies idempotently before tokenization):**

A geometry's token sequence has multiple arbitrary degrees of freedom that an upstream pipeline can permute without changing the geometry's identity. The canonical form pins each DOF:

| Geometry class | DOF | Canonical rule |
|---|---|---|
| `LineString` (open or closed) | direction | **PRESERVE source direction** (no canonicalization). LineString vertex-traversal direction carries semantic information that BP1 vocab does not capture — see "Open-polyline direction preservation" evidence note below. |
| `Polygon` exterior + each interior hole (closed rings) | start vertex | Rotate the ring so it begins at its lex-min vertex (excluding the duplicate closing vertex from the comparison). |
| `Polygon` exterior + each interior hole | winding | RFC 7946 right-hand rule: exterior CCW, interior holes CW. If a ring's signed area (via shoelace) has the wrong sign for its role, reverse it. Polygon rings carry NO traversal semantics: the inside/outside is determined by winding alone, so RFC 7946 canonicalization is pure-redundancy reduction. |
| `MultiLineString` / `MultiPolygon` | part order | After each part is internally canonicalized, sort parts by their first vertex. For MultiPolygon parts this equals the lex-min vertex by construction; for MultiLineString parts (whose internal direction is preserved, not canonicalized) it equals the source's first vertex. |
| `MultiLineString` / `MultiPolygon` tie | tiebreak | When two parts share an identical first vertex, fall through to the next vertex in source-order (LineString) or rotated order (Polygon); if still tied, compare by part vertex count (smaller first). |
| `Point`, `MultiPoint` | (none) | Trivially canonical. |

**Idempotency:** the canonicalizer's output is invariant under re-application. Re-canonicalizing an already-canonical geometry returns it unchanged. This is what makes "implement idempotently regardless of upstream" the right framing: if Overture or sub-A ever starts enforcing RFC 7946, sub-F's winding rule is a no-op on that input.

**Open-polyline direction preservation (evidence, 2026-05-28).** A grep of `configs/sub_f/semantic_vocab.yaml` (127 LOCKED slots: 28 L1 wildcard keys + 99 L2 highway/building pairs + Singapore X pass-list) found NO direction-encoding tokens: no `oneway=*`, no `direction=*`, no `bridge=yes`, no `cycleway:left|right`, no `waterway` flow tags. The only direction-adjacent match (`building=bridge_structure`) is categorical, not traversal. Therefore vertex-traversal direction on open polylines (and closed LineStrings such as roundabouts) carries OSM semantics that are NOT recoverable from sub-F's token sequence — `highway=motorway` traversal direction encodes legal travel direction (motorways are oneway by OSM convention even when `oneway=yes` is not explicitly tagged); `natural=coastline` traversal puts land on the right by OSM convention; `waterway=river` traversal goes downstream. If sub-F canonicalized open-polyline direction, this information would be IRRECOVERABLE after training. Sub-F output IS the training input. Cheap-to-keep applies: preserve direction at the encoder, accept the small entropy cost, leave room for a future `oneway`-aware vocab without re-tokenizing the dataset. Closed `LineString` (start == end, e.g., roundabouts) is treated as a LineString for this rule, not as a polygon ring, because roundabouts often carry oneway semantics that ring-rotation would destroy.

**Upstream winding behavior (evidence, 2026-05-28):** neither sub-A (`src/cfm/data/overture/`) nor sub-C (`src/cfm/data/sub_c/`) enforces RFC 7946. No `orient(`, `winding`, `right-hand`, or `signed_area` calls. `src/cfm/data/sub_c/io.py` `dump_wkb` uses `wkb.dumps(geom, hex=False, byte_order=1)` — explicit NDR byte order, no winding normalization. Shapely preserves winding through WKB round-trips. Overture source WKB winding is provider-determined. Therefore sub-F's winding canonicalizer is the PRIMARY enforcement site, not just a backstop. (Polygon rings carry no traversal semantics, so winding canonicalization is safe — distinguishable from the LineString direction case.)

**Why CANONICALIZE beats INHERIT (recorded for §13.1):** INHERIT would have staked sub-F's byte-determinism contract on the stability of undocumented DuckDB httpfs partition-scan ordering through Overture parquets. The script's 20/20 sub-C round-trip match documents only that pyarrow is bit-stable on the same file — a trivially-true property of one chain hop. CANONICALIZE makes sub-F's output order-invariant to the entire upstream chain, paying one encoder helper + one per-axis test for permanent immunity to a class of defects.

**T5b per-axis tests** (one per DOF, plus tiebreak) cover each rule with adversarial inputs that hold other DOFs constant — see plan Task 5b Test 5 split.
...
| **Halt 5 review → BP5:** vertex-order canonical form expanded from 1 DOF to 4 (open-polyline direction + closed-ring start + closed-ring winding + multi-part order with tiebreak) | Reviewer ratified Task 5a's (a)→(c) reclass (the verification script tested only the sub-C↔parquet hop, 1 of 3 chain hops). Plan's "lex-min polygon-ring rotation" framing was itself the same defect shape — it pinned 1 of 3 arbitrary DOFs in a ring's token sequence (start vertex) while leaving winding and multi-part order unconstrained. Upstream-winding fact-find (2026-05-28) confirmed neither sub-A nor sub-C enforces RFC 7946, making sub-F's winding canonicalizer PRIMARY rather than backstop. | §5.2 row "Vertex iteration within feature" locks to CANONICALIZE; §5.6 enumerates the 4-DOF canonical form with deterministic tiebreak; T5b Test 5 splits into 5 independent assertions (one per DOF + tiebreak); Task 8 encoder ships `canonicalize_geometry()` as a primary step before tokenization. Rationale recorded: INHERIT would have staked byte-determinism on undocumented DuckDB httpfs ordering; CANONICALIZE makes output order-invariant to the upstream chain at the cost of one encoder helper + one per-axis test. | §5.2 + §5.6 + §13.5 |
| **Halt 5 review → BP5:** rounding mechanism rationale corrected away from `frequency_analysis.py:721` / `evidence.py:359` precedent | Task 5a's expanded grep found these two `int(round(...))` sites in sub-D, but both quantize percentile-indices (integer slot picks from already-sorted lists), not coordinates. Citing them as load-bearing precedent would have conflated index-quantization with coordinate-snapping semantics. | §5.2 row "Round tie-breaking" rationale rests on: (a) determinism requires one rule pinned, (b) banker's is bias-free for coordinate snapping. Sub-D `int(round(...))` usage recorded as house-style consistency only. Adjacent: sub-C `coords.py` `math.floor` for cell-binning + sub-F `round` for coordinate-snapping is correct-by-operation (floor bins, round snaps), not a latent inconsistency. No §9.6.1 cascade. | §5.2 + §13.5 |
| **Halt 5 review → §9 cite drift:** sub-C feature sort key cite amended `:218-220` → `:219-221` | Task 5a audit found the cited line range was off by one from the actual `sorted(features, key=lambda r: (...))` block. T5b string-match test is line-agnostic and was unaffected, but the documentation cite drifted silently. Surfaced as forward preference: prefer content-anchored cites over line numbers (`feedback_content_anchored_cites`, 2026-05-28). | §5.2 row "OSM feature iteration order" cite updated. §9.1–§9.5 line-number sweep deferred as a separate atomic commit, not bundled with this lock. | §5.2 + §13.5 |
| **Halt 5 follow-up → BP5 canonical form:** open-polyline direction DOF DROPPED from canonicalization (preserve source) | Reviewer-stated rule: lex-min ring rotation / winding / multi-part order are pure-redundancy DOFs (same geometry, different bits); open-polyline direction is NOT — vertex traversal carries semantic info for oneway roads, coastlines, rivers, etc. Verification gate: does BP1 vocab capture direction? If yes → safe to canonicalize. If no → cheap-to-keep applies; canonicalizing destroys recoverable info irrecoverable after training. BP1 grep (2026-05-28, 127 LOCKED slots) found NO direction-encoding tokens — no `oneway`, no `direction`, no `waterway`-flow, no `cycleway:left/right`. Sole direction-adjacent match `building=bridge_structure` is categorical, not traversal. | §5.2 row "Vertex iteration within feature" amended to enumerate the 3 pure-redundancy DOFs + explicit open-polyline preservation. §5.6 canonical-form table updated: LineString rule = PRESERVE source direction; added "Open-polyline direction preservation" evidence note. §13.5 protocol-bump candidate: cheap-to-keep gate for canonicalization in DATA-LAYER decisions (not just schema-layer) — should appear as a brainstorm checkpoint whenever an "irrecoverable after training" lossy step is proposed. Closed-LineString (e.g., roundabouts) is also preserved per the conservative cut (oneway semantics possible). | §5.2 + §5.6 + §13.5 |
| **Halt 4 ratification → BP3 budget:** P99.9 elbow + per-type floor CALIBRATION (not POI relaxation) + α truncation + token-anchored long-cell diagnostic with defined action contract | Halt 4 ratified with five reviewer modifications to the implementer surface: (1) PRD 10K projection dropped from Halt 4 scope (premise error — 10K is BP1 vocab floor / training-set tile count, not sequence-length ceiling; per-cell budget by §7.1 architecture); (2) per-type floors reframed from "POI relaxation" to category-error correction — §7.5 uniform 99% floor mis-specifies POIs because their loss profile is per-cell (dense-cluster) not per-feature (sparse); locked at observable retention given P99.9 budget; (3) α chosen on not-reopening-§5.2/BP5-locks rationale, not on simplicity; (4) long-cell diagnostic re-anchored from spec §7.7 percentile-space (`chosen_quantile - 0.5pp = P99.4 = 4096 tokens`, 1792 tokens below budget = noise) to token-space (2 padding blocks below padded budget = 5,632 tokens, 6 cells/region run); (5) diagnostic gained a defined action contract (per-run logging + revisit triggers + revisit actions) — "a threshold that fires is inert unless firing MEANS something." Three premise checks during ratification: <unknown_*> token accounting verified safe (no recompute); α drop report computed (31 cells dropped at raw, 30 at padded, POIs 10% drop, dropped-cell median 6,719, distribution dominated by 15-52% over-budget tail); stage-4 headroom quantified (96-token slack vs 3-7 token plausible error vs 1 cell in elbow band = 13-32x margin). | `configs/sub_f/sequence_length_analysis.yaml _status: LOCKED` with full lock block. α drop tooling at `scripts/sub_f/compute_alpha_drop_report.py` + 2 durable drop reports. Close-checklist + 3 recheck obligations (re-run T3c with measured stage-4 when sub-E lands; regenerate α drop report per region/release; evaluate warning-band growth across runs). | §7 + §10.1 Halt 4 row + §13.5 |
| **Halt 4 ratification → §13.5 β-upgrade-case honesty:** α drop distribution weakens the β-upgrade case vs naive "keeps dense commercial cells" framing | α drop report (Premise B during Halt 4 ratification) showed dropped-cell length distribution at padded 5,888: min 5,913 / median 6,719 / max 8,966. Cells are 15-52% OVER budget, not hovering at the elbow. β recovers within-cell tails by dropping low-priority features, but β would STILL truncate most of these cells because their over-budget magnitude is large (a 6,700-token cell needs ~13% feature drop to fit; 9,000-token cell needs ~35% drop, which defeats the purpose). α vs β barely differ in retention on this drop set. β-upgrade scaffold preserved in the lock's action_contract, but recorded honestly: marginal-retention-gain expectation should be calibrated against the distribution, not against the "dense commercial cells" framing. Re-measure when sub-E lands and multi-region data exists. | Don't let the β case read stronger in the spec/plan record than the Singapore distribution supports. β-upgrade scoping happens at training-scaffold sub-project IF the drop-set composition across regions justifies it; lower priority than the original Halt 4 implementer surface implied. | §13.5 |
| **Halt 4 ratification → §13.5 diagnostic-action-contract protocol-bump candidate:** every diagnostic threshold must carry a defined action contract | Halt 4's long-cell diagnostic shipped (in the implementer surface) as `proposed_long_cell_diagnostic_pp: 0.5` with no defined response semantic — a number that logs to a YAML field and never triggers any action. Reviewer push: "A threshold that fires is inert unless firing MEANS something. Put the action in the lock next to the threshold." Captured in `feedback_diagnostic_threshold_design`. Proposed protocol-v2 addition: any diagnostic / monitoring threshold lock must include (a) trigger condition (count / rate / trend), (b) specific response action (revisit, halt, escalate, recheck-when-X), (c) owner of the response. Without (a)+(b)+(c) the threshold is inert; promotion to a brainstorm/lock-time gate prevents this failure mode. | Surface in sub-F-close protocol-bump candidates section + sub-F-close handoff. | §13.5 |
| **T8 plan-write → BP2 inventory:** structural-sentinel ID gap discovered + resolved append-safely (Halt-2-cascade) | T8 plan-write self-review surfaced spec §2.2 / sentinel_inventory.yaml inconsistency: `<feature>` / `<feature_end>` were named in §2.2's "Structural sentinels (on-disk; 6 named)" row but had no allocated ID in any locked inventory sub-block. Investigation triggered an append-safety analysis across all reserved spaces (BP1 tail, BP4 tail, dataloader reserved, BP2 reserved_v2_headroom, BP7 tail). Three locked-in-use placements (BP2 anchor 300/301, anchor 394/395, magnitude end) explicitly rejected per append-only discipline: reassigning locked anchor coords mid-phase would silently contradict the locked inventory. Reading-correctness gate: option-4 reading (only `<feature>` / `<feature_end>` are real sentinels; the other 4 §2.2 names are positional descriptors of value-token classes) was forced by consistency with BP2 (Halt 2 hierarchical encoder emits value tokens directly, not marker+value pairs) AND BP3 (Halt 4 budget surface measured against marker-LESS grammar; 6-sentinel reading would retroactively invalidate both locks). Self-delimiting check at T8 plan-write verified that the grammar parses unambiguously without marker tokens via token-class ID ranges + positional convention + strict (dir, mag) alternation. | Resolved by consuming IDs 509, 510 from the FRONT of BP2 `reserved_v2_headroom` (now 511–1499); block boundaries NOT re-anchored — IDs consumed from front per append-only precedent. Family classification: `structural` (grammar primitives) — distinct from `encoding_primitive` (value tokens); ID locality and family classification are allowed to differ. Lock changes: `configs/sub_f/sentinel_inventory.yaml` adds `bp2_encoding_primitives.consumed_from_reserved_v2_headroom.slots` (2 entries) + updates `used_count: 209→211`, `used_range: 300..508→300..510`, `reserved_count: 991→989`, `reserved_v2_headroom.start_id: 509→511`, `reserved_v2_headroom.slot_count: 991→989`. §2.2 row reworded as documentation correction (no ID renames, no reorders, within-phase legal). | §2.2 + §13.5 + sub-F-close handoff. |
| **T8.7 plan-write → BP7 bref vertex position: NO v1 round-trip gate (v2-scoped deferral per §1.4 scope lock #1)** | T8.7 plan-write surfaced a Halt-2 lock gap masquerading as one: spec §3.8 round-trip §2 paired check enumerates four cases ("ALL must pass independently per BP2 fix 1"), but case 4 (road crossing cell boundary) wording reads *"admits larger L_∞ tolerance for position quantization at the edge; class agreement remains strict."* The Halt 2 lock at `configs/sub_f/encoding_primitives.yaml` carries only one threshold (`round_trip_l_inf_threshold_m: 4.8`); no case-4 number was locked. The gap is NOT a missing number — it is a v2-scoped deferral. Spec §1.4 scope lock #1 already pins this: *"Position-aware boundary contracts. Sub-E v1 contract is class-only; exact crossing positions deferred to sub-E-v2 + sub-F-v2 joint sub-project (§12 #1). Sub-F-v1 ships class-only `<bref_dir_class>` tokens."* Encoder Case B/D DROPS the final vertex (replaces final dir+mag with `<bref>`); decoder cannot reconstruct the crossing position from a class-only token. The §3.8 case-4 "larger tolerance" wording is interpretive, not a number-to-lock. | **Position deferral framing (v2-scoped, NOT temporary):** sub-F-v1 ships NO position gate on the bref vertex. The bref edge vertex's geometric error is **unbounded-by-test in v1, by design**. The honest BOUND is `cell_extent/2 = 125m` (worst case: anywhere on a 250m edge); recorded here as the v1 imprecision bound, NOT as a tested threshold. **v1 BP7 coverage** is class-agreement only — §8.1 BP7 row's four-test composite (T8.5 cross-reference, T10 cross-tile validator's symmetry + non-road-non-emission + coverage). **v2 obligation:** when sub-F-v2 (joint with sub-E-v2 per §12 #1) ships exact crossing positions, the geometric round-trip gate is added at that point with a threshold derived from sub-E rotation precision. T8.7 plan adds a NEGATIVE test asserting Case B/D decoder output is NOT round-trip-asserted on the bref vertex (explicit in code so a future contributor cannot silently re-add the assertion thinking it was forgotten) with `# v2-scoped deferral per spec §1.4 + §13.1 ledger` comments at every bref vertex emission site. | §1.4 + §3.7 + §3.8 + §8.1 BP7 row + §12 #1 + sub-F-close handoff. |
| **Halt 4 revisit → BP3 budget RE-LOCKED for encoder chunking discovered at T8.7 dispatch: 5,792→5,899 raw / 5,888→6,016 padded** | Surfaced when the T8.7 aggregate stress test L_∞ = 183m revealed the encoder silently dropped spec §3.5 magnitude-chunking on segments > 32m (fix at 86f0c99). The BP3 budget surface had been measured against the UNCHUNKED Case-A formula; the chunked audit (`scripts/sub_f/audit_chunked_budget.py`, `reports/sub_f_chunked_budget_audit.yaml`, commit 2562c89) showed chunked P99.9 raw shifted 5,792→5,899 (+107, exceeding the 96-token padding slack by 11) and padded 5,888→6,016 (+128, one padding block up). Per-type chunked inflation: roads +13.8%, buildings +3.4%, POIs +0%, base +26.5%. This is a spirit-of-recheck-obligations revisit per the existing lock's `recheck_obligations` block (chunking was simply undiscovered at Halt 4, so it was not a named trigger), NOT a new design. | Lock re-locked at 5,899 raw / 6,016 padded (`configs/sub_f/sequence_length_analysis.yaml lock.revisit_2026_05_29`). Per-type floors recomputed from the chunked PADDED-budget α cut — all drifted UP or flat (roads 99.34→99.36%, buildings 98.83→98.89%, POIs 90.18→90.27%, base 99.92% unchanged): the +128-token padded budget rise outpaced chunking inflation at the tail. Long-cell diagnostic MECHANICALLY re-anchored 5,632→5,760 (2 padding blocks below the new padded budget, per the locked action contract — not a reviewer re-derivation). Padding slack 96→117 tokens; elbow-band cell count 1→2. Per-feature cost extracted to `cfm.data.sub_f.token_cost` (pinned against the encoder in `tests/data/sub_f/test_token_cost.py`) and shared by the α drop report so the budget formula and the emitter cannot drift silently again (the failure mode this revisit corrects — `feedback_test_spec_not_just_plan`). Emitter-faithful re-verification (`scripts/sub_f/verify_chunked_budget_p999.py`) confirms the locked padded budget holds under per-part Multi* costing (CONCAT P99.9 = 5,899 raw reproduces the audit; SPLIT = 5,900 raw; both round to 6,016 padded). Resolution rationale: +128 tokens (~2% attention FLOPs) is trivial vs the silent under-coverage on the road/base-heavy tail that staying at 5,888 would cause (the cheap-to-keep violation pattern); chunked-P99.99 option rejected as overfitting. | §3.5 + §7 + §13.1 + sub-F-close handoff. |
| **Halt 2 revisit → BP2 direction RE-LOCKED 48→360; round-trip thresholds re-derived fresh (position p99.9 ≤ 4.8m, angle p95 ≤ 4.0°)** | T8.7's decoder round-trip gate exposed that the BP2 thresholds did not hold on real data. Root cause is `feedback_sample_regime_blind_locks` in two forms: (1) the 4.8m position bound + 7.5° angle bound were the **p95 of a 1,000-polyline sample** (`l_inf_scope: polyline_only`; sample max already 12.9m) that under-represented the real 862k-feature distribution; the chunking fix (86f0c99) then made long segments faithfully representable and unmasked the gap (real honest position p95 = 8.8m, p99.9 = 27.9m at N=48). (2) The T8.7 gate enforced 4.8m as a *max* while it was *derived* as a p95 — a derivation/enforcement statistic mismatch. Additionally 7.5° was just the N=48 bin width, a gate nothing could fail. Scoping (`reports/sub_f_halt2_revisit_scoping.md`) measured the full fix space on real data. | **Direction widened 48→360 (1° bins)** — the ONLY mechanism that improves BOTH axes (position p99.9 8.8→**3.7m**, angle catastrophic corners 3,500→1,558) because it refines quantization without restructuring relative vertex placement. Error-feedback (angle catastrophic 10×) and re-anchor (angle catastrophic 2.2× + a 3rd BP3 budget pass) were scoped and REJECTED — both restructure placement and regress the angle axis. Thresholds re-derived FRESH (not inherited from the weak sample-p95): position **p99.9 ≤ 4.8m** (measured 3.7m via actual encoder+decoder on real data), angle **p95 ≤ 4.0°** (measured 3.0°). The ~14m position **max** is an accumulation floor inherent to any direction-quantized + single-anchor scheme (continuous loss, NOT the categorical right-angle known-loss — `feedback_forced_precedent_tell`), documented as v1 known-loss; a max gate is unachievable cheaply by any mechanism. **Append-safe ID relocation**: 360 contiguous slots don't fit at the old direction base 396 (magnitude 444-508 + structural 509-510 follow it), so the direction sub-block relocates to **511-870** (front of reserved_v2_headroom, the structural-sentinel precedent); old 396-443 retired to `direction_v1_deprecated` (NOT reused — reusing would reassign locked IDs); magnitude/structural/anchor/BP7 unchanged; reserved 989→629; on-disk vocab 374→686. **BP3 budget unaffected** (tokens-per-pair unchanged — direction count doesn't change token count; pinned via `test_token_cost.py`). Enforcement: position synthetic gate binds in `test_decoder.py` (aggregate max ≤ 4.8m; failed at 25.8m on N=48); the angle gate is NOT unit-tested (its failure mode is data-dependent and a uniformly-rotated synthetic rectangle preserves exact-90° corners at every N — false assurance) and binds via the YAML value + real-data measurement + a close-checklist re-measure obligation, as the original 7.5° did. | §3.4 + §3.8 + §8.1 BP2 row + `encoding_primitives.yaml proposed_lock.revisit_2026_05_29` + `sentinel_inventory.yaml direction_relocation_2026_05_29` + sub-F-close handoff. |
| **T11 dispatch → §4.2 per-cell `provenance_sha256`: resolved to content-equivalence anchor (v1); cross-layer lineage deferred to v2** | T8.8 wrote the per-cell `provenance_sha256` column as `sha256(struct.pack(">{n}H", *tokens) + bytes([cell_i, cell_j]))`, labeled "stub for Task 11 to overwrite." Spec §4.2 "sha chain anchor" is ambiguous between (1) a **content-equivalence anchor** (hash of what was emitted) and (2) a **provenance-chain anchor** (upstream lineage to sub-C `source_feature_id`s + sub-E digest). Resolved to (1) for v1: byte-determinism (locked at T5b) directly implies per-cell sha stability across re-runs, which is the integrity property v1 needs; (2) is a larger architectural choice (moves the per-cell hash dependency from "what was emitted" to "what was read upstream") that must not be embedded in T11 without spec guidance — it is v2-shaped. T11-dispatch Step-0 reverified two properties before locking the interpretation: **(a) byte-determinism** — endianness is pinned big-endian (`>H`), coords are integer `cell_i`/`cell_j` (not floats), so the hash is platform-stable (covered by T5b's cold-subprocess `encode_tile` byte-identity test); **(b) uniqueness contract** — the per-cell sha is content-equivalence, NOT a unique identifier (T9 inline checks FORMAT only via `^[0-9a-f]{64}$`; T10 `_check_sha_uniqueness` operates on the TILE-level `provenance.yaml` sha). Two byte-identical cells across tiles legitimately share a per-cell sha by design; nothing downstream relies on per-cell uniqueness. | The "stub for Task 11 to overwrite" framing is DROPPED — the T8.8 content hash IS the v1 per-cell anchor and T11 keeps it unchanged. T11 owns only the TILE-level `provenance.yaml` `provenance_sha256` (via `provenance.py::provenance_sha256` over a tile-identity-bearing provenance dict, so distinct tiles → distinct tile shas) + the region manifest. Cross-layer provenance chaining (per-cell lineage to sub-C/sub-E) deferred to a v2 spec revision when full chain semantics are designed; this v1 anchor satisfies determinism + integrity but does not encode upstream lineage. | §4.2 + §6 + sub-F-close handoff (v2 cross-layer-chaining deferral recorded here). |
| **T12 dispatch → CLI user-contract (encode/decode/validate/derive): defined where spec §11.1 only named the four `scripts/sub_f/*.py` files** | Master-plan Task 12 framed the four CLIs as a "light wrapper, no halt," but the elevated halt posture (glue-over-verified-pieces is the TRIGGER) + a mandatory Step-0 signature reverify found every wrapped function had changed shape this session and that encode/decode had NO defined user-contract. Step-0 stale-snippet catches (own-session-generated staleness, `feedback_reverify_plan_snippets_at_dispatch`): (a) `validate_cross_tile` is now two-arg `(sub_f_region_dir, sub_e_region_dir)` — the plan's one-arg call would `TypeError`; (b) `PipelineConfig(**vars(args))` works only by accident (arg dests must exactly equal the 6 field names with no extra arg present); (c) the plan snippet's "Mirrors sub-E `scripts/sub_e/derive.py`" docstring is dead — sub-E lives at `scripts/derive_boundary_contracts.py`; (d) the encode CLI's `--inbound/--outbound-bref` cannot be a bare CLASS — `encode_feature` feeds the value straight into `tag_to_id[...]` and a bref encodes BOTH direction and class; (e) `encode_feature` has NO `cell_origin` parameter (only `encode_cell` does); (f) `resolve_bref_tag` does NOT raise on a bad class — it `return None` for any non-emitting class (the `if class_label not in _EMITTING_CLASSES` branch precedes the direction check), so "let resolve_bref_tag raise" covers bad DIRECTION only. | encode/decode scoped as DEVELOPER-INSPECTION tools (per-feature granularity), NOT v1 pipeline consumers — the pipeline persona uses derive.py + validate.py. **Bref flag = DIR:CLASS** (single cardinal letter N/S/E/W + canonical class name MAJOR_ROAD/MINOR_ROAD) resolved via `resolve_bref_tag` (the production path); literal-vocab-tag form rejected (leaks vocab internals to the user contract, fragile across BP7 re-keys) and separate dir/class flags rejected (fragments the bref direction-class unit, creates an invalid-combination surface). The CLI adds a flag-provided⟹non-None guard for the bad-class case (catch (f) above). **`--cell-origin x,y` = CLI-side pre-translation** (`translate(geom, -x, -y)` before canonicalize + encode), faithful to how `encode_cell` positions a feature within its cell. **Bare-geometry contract:** decode emits and encode consumes a bare GeoJSON geometry (NOT a Feature wrapper), matching `decode_feature`'s return shape and enabling `decode … | encode …` stdin/stdout round-trip (pinned by a fast in-process test + a `@slow` real-OS-pipe test). `--round-trip` reports L_∞ vs the CANONICALIZED input to stderr (tokens stay on stdout); Cases B/C/D are intentionally lossy on the v2-scoped bref vertex per §1.4 and the diagnostic says so. validate gains required `--sub-e-region-dir`; derive gains optional `--extracted-utc` (byte-reproducible runs) + `--no-alpha-drop-report`. SEVENTH `feedback_ambiguous_spec_resolved_silently` instance this session — Step-0 caught the unimplementable `--inbound-bref CLASS` before the implementer shipped a `tag_to_id` KeyError. | §11.1 (Task 12 row) + §3.7 (bref) + §1.4 (v2 bref-vertex deferral) + §4.2 (bare-geometry) + sub-F-close handoff. |
| **T14 dispatch → empirical gate scoped as T13's COMPLEMENT (per-type retention only); + DUAL-LOCK read forward-warning on the companion lock YAMLs** | Master-plan Task 14 (written pre-T13) re-implemented round-trip 4-case + BP7 four-test composite on real Singapore — but T13 had JUST consolidated exactly those, so T14 would duplicate them (instance 9, structurally identical to T13's instance-8 stub duplication). Step-0 also found the plan's retention thresholds hardcoded the PRE-Halt-4 §7.5 defaults (roads 0.999 / others 0.99 — which survive in `sequence_length_analysis.yaml` as `retention_defaults_per_spec_7_5` for audit), NOT the operative re-locked floors (roads 0.9936 / buildings 0.9889 / pois 0.9027 / base 0.9992); a real POI retention ~0.903 would FALSELY FAIL the plan's 0.99 while clearing the 0.9027 lock. The 4dp-stored floors also trip a naive `measured >= floor` (roads true 0.99356 < stored 0.9936). | T14 = the per-type RETENTION gate ONLY (the one locked surface T13 does not verify); round-trip/BP7 NOT re-implemented (T13 owns them). The gate READS floors + budget from the operative `lock.*` block (never hardcodes a locked number), with a half-ulp tolerance (5e-5) for the 4dp floor storage. Retention is sub-C-derived (chunked cost + §7.2 formula stage-4) so it RUNS on the present sub-C Singapore cache today (~14s, passes — regression-guards the floors against encoder/budget drift) and fails-loud if sub-C absent; the empirical-stage-4 re-measure + committed golden are deferred to sub-E-landing (close-checklist). **DUAL-LOCK FORWARD-WARNING (structural hazard, NOT T14-specific):** both companion lock YAMLs carry a HISTORICAL block shadowing the OPERATIVE one — `encoding_primitives.yaml` `approved_lock` (direction 48 / angle 7.5, SUPERSEDED) vs `proposed_lock.revisit_2026_05_29` (360 / 4.0, operative); `sequence_length_analysis.yaml` top-level `budget_surface`/`retention_by_quantile` (pre-chunking 5792/5888, per its `_prechunking_analysis_note`) vs `lock.*` (5899/6016 + floors, operative). ANY consumer MUST read the operative block — a naive top-level read returns the superseded value. | §7 + §3.4/§3.8 + §11.1 (Task 14 row) + `sequence_length_analysis.yaml lock.*` + `encoding_primitives.yaml proposed_lock.revisit_2026_05_29` + sub-F-close handoff. |

Fifteen cross-bite-point revisions; five surfaced during brainstorm topic-by-topic gating, ten more surfaced at plan-write/dispatch time (pre-dispatch audit + prompt-derivation review + reviewer-side redispatch check + Halt 1 review + Halt 7 integration review + T11-dispatch interpretive resolution + T12-dispatch CLI-contract resolution + T14-dispatch retention/dual-lock resolution). The ten post-brainstorm revisions are defense-in-depth evidence: brainstorm Gate 6 + plan-write audit + prompt-derivation reviewer pass + reviewer-side redispatch check + halt read-through = five-layer redundant coverage on upstream-contract verification. See §13.5 protocol-bump candidate aggregation.
