...
| Vertex iteration within feature | **LOCKED at Halt 5 (2026-05-28) as CANONICALIZE** per `feedback_ambiguous_third_branch_in_verification` defend default. The Task 5a verification covered only the sub-C ↔ parquet round-trip hop (1 of 3 chain hops); the Overture → sub-A hop is not documented as vertex-order-preserving (`docs/data/overture_pinning_policy.md` does not commit to it). Canonicalization makes sub-F output order-invariant to the whole upstream chain — INHERIT would have staked byte-determinism on undocumented DuckDB httpfs ordering. Canonical form specified in §5.6; encoder implements idempotently. | Task 5a evidence: `reports/2026-05-23-phase-1-sub-F-task-5a-halt.md` + `reports/sub_f_task_5a_vertex_order.yaml`. |
...
|---|---|---|
| `LineString` (open polyline) | direction | Pick lex-smaller of `tuple(forward_coords)` vs `tuple(reverse_coords)`. |
...
| `Polygon` exterior + each interior hole | winding | RFC 7946 right-hand rule: exterior CCW, interior holes CW. If a ring's signed area (via shoelace) has the wrong sign for its role, reverse it. |
| `MultiLineString` / `MultiPolygon` | part order | After each part is internally canonicalized, sort parts by their first vertex (which by construction equals their lex-min vertex). |
| `MultiLineString` / `MultiPolygon` tie | tiebreak | When two parts share an identical first vertex, fall through to the next vertex in rotated order; if still tied, compare by part vertex count (smaller first). |
