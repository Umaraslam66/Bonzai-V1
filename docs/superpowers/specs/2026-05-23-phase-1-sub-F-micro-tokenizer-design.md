...
**Class assignment semantics (cascade #9).** BP7 tokens represent **drivable-network continuity for AV routing**, not every OSM `highway=*` value that touches a cell edge. Sub-F composes sub-E's grouping with a sub-F-local override map: values already grouped by sub-E remain inherited (`primary`, `secondary`, `trunk` → MAJOR; `tertiary`, `residential`, `service`, `unclassified`, `footway`, `steps`, `cycleway` → MINOR). Locked BP1 values absent from sub-E grouping resolve explicitly in sub-F, never by omission: `motorway` → MAJOR; `living_street` → MINOR; `subway`, `path`, `track`, `pedestrian` → NONE deliberately (non-drivable for AV route continuity); `motorway_link`, `primary_link`, `secondary_link`, `tertiary_link`, `trunk_link`, `bridleway`, `busway`, `road`, `*` → NONE as Singapore scope-of-coverage-zero at Halt 7. Tests assert every locked BP1 `highway=*` value resolves to exactly one `BoundaryClass`.
