...
**Class assignment semantics (cascade #9, PROVISIONAL pending Halt 7 reclassification).** Halt 7 surfaced a composition mismatch between locked BP1 `highway=*` values and sub-E grouping. A sub-F-local drivable-continuity override was explored, but it is not locked: the BP7 purpose question (drivable-routing continuity vs geometric continuity) and derivation architecture (sub-F class map vs authoritative sub-E `boundary_contract.parquet`) must be resolved before class assignments lock.
...
Twelve entries enumerating what sub-F-v1 does NOT ship — to be reopened by a future sub-project. Within-sub-F verify-before-lock pending references (sub-C feature-splitting, `compare_version` fallback, sub-D `round()`) are inventoried at §9.6, not here. §12 is cross-sub-project deferrals only.
...
| Other-sub-project (sub-D-v2 / sub-E-v2) | 1 | #7 |
...
| **Halt 7 review → BP7:** sub-E grouping under-covered locked BP1 highway vocab (cascade #9) | Halt 7 audit found sub-E's grouping omitted 15 locked BP1 `highway=*` values. The raw gap was WIDE, but per-value review split it into drivable continuity (`motorway` → MAJOR, `living_street` → MINOR), deliberate non-drivable non-emission (`subway`, `path`, `track`, `pedestrian` → NONE), and Singapore scope-zero values (`*_link`, `bridleway`, `busway`, `road`, `*` → NONE). | Sub-F owns a local BP7 drivable-continuity override composed after sub-E grouping. Every locked BP1 highway value resolves explicitly to one `BoundaryClass`; NONE is deliberate, never omission. sub-E remains unchanged. | §3.7 + §8.1 BP7 row + §13.5 |
...
- **(vi-b) Reviewer-supplied premise verification repeats at Halt 7** (additional evidence for candidate vi). BP7 cascade #9 initially accepted a reviewer-supplied "drivable-only" purpose premise without grounding it in the pre-existing §3.7 text or derivation architecture. The premise conflicted with the defer-to-sub-E behavior for `footway/cycleway/steps` and had to be pulled back. Reviewer assertions about purpose and semantics are untrusted input just like reviewer-supplied parameter values.
...
- **(x) Late-stage integration composition audits** (proposed protocol-v2 addition). Late-stage integrating sub-projects should budget explicit composition-audit tasks against each upstream they consume because per-upstream correctness does not compose automatically. Sub-F has now surfaced multiple upstream-composition gaps: cascade #7 (sub-C normalization sentinels admitted to BP1 X pass-list), cascade #9 (sub-E grouping under-covering locked BP1 highway vocab for sub-F's drivable-continuity semantic), and the Task 7 MultiLineString surface (sub-C MultiLineString rows versus §3.7 single-line assumption, pending reviewer classification). Pattern: sub-F as the late-stage consumer pays the composition tax for locally-correct upstream artifacts.
