...
|---|---|---|---|---|---|
| 1 | **BP1 vocab floor elbow** | Task 1 | Marginal-cost curve over L1/L2/L3 granularity (level, vocab_size, must-appears_admitted); X-threshold candidates for Singapore-prioritized must-appears | Granularity level + sub-floor exception list + X-threshold value + X-threshold paired structural check framing | `configs/sub_f/vocab_floor_analysis.yaml`, `configs/sub_f/semantic_vocab.yaml` |
...
| Sub-F-v2 standalone | 3 | #6, #8, #9 |
| Other-sub-project (sub-D-v2 / sub-E-v2) | 1 | #7 |
| Multi-region expansion | 1 | #5 |
| End-of-Phase-1 verification pass | 1 | #4 |

Three of nine (33%) lock sub-F-v2's scope upfront (#6, #8, #9) — exactly the kind of forward-looking residual list `feedback_append_only_vocab_safety` + `feedback_schema_vs_data_cost_asymmetry` argue should be tracked at v1 close, not rediscovered at v2 open.
