...
|---|---|---|---|---|
| 1 | v2 vocab + derivation expansion. Five candidate axes documented from Task 14 pre-dispatch audit's empirical class_raw distribution on the Layer-3 9-tile subset (1008 candidate edges; 709 crossed; class_raw value distribution: 942 `service`, 919 `footway`, 626 `residential`, 217 `subway`, 169 `secondary`, 154 `primary`, 151 `motorway`, 107 `cycleway`, 91 `unknown`, 74 `trunk`, 58 `tertiary`, 48 `track`, 37 `pedestrian`, 31 `unclassified`, 29 `steps`). All non-v1-vocab values currently route to MINOR_ROAD via spec §5.1's default bucket — informative but lossy. Candidate axes for v2 deliberation: **(a) NONE split** — `NONE_INTERIOR` (active, no crossings) vs `NONE_NATURAL_BOUNDARY` (active, only non-road crossings like water/rail/coastline) so the model can learn boundary-vs-interior distinction; **(b) highway-tier promotion** — `motorway` (151 obs) and link variants (`motorway_link`, `primary_link`, `trunk_link`) currently default-bucketed to MINOR; promote to MAJOR_ROAD or introduce a SUPER_ROAD class; **(c) underground transit** — `subway` (217 obs) currently MINOR_ROAD by default, but subway tunnels don't form visual urban boundaries; consider excluding entirely or new transit class; **(d) pedestrian-class consideration** — `footway` (919), `cycleway` (107), `steps` (29), `pedestrian` (37) currently in MINOR_ROAD via vocab; consider separate PEDESTRIAN class so the model can distinguish vehicular vs non-vehicular ROW; **(e) rail handling** — `track` (48 obs) and any future `rail` class_raw currently MINOR by default but semantically structural (built infrastructure forming urban boundaries). | §5; Task 14 pre-dispatch audit (commit 4a31e77) empirical class_raw distribution | Post-reset boundary-quality cycle, OR empirical evidence that v1 NONE contaminates the perplexity gap, OR empirical evidence that any single axis (motorway, subway, pedestrian, rail) carries signal v1's MINOR_ROAD default-bucket cannot reach | `boundary_vocab_version` + `boundary_derivation_version` bump; new class-grouping map (likely expanded class set, not just remapped); re-derive sub-E for all tiles. Bumps are per-namespace per spec §9.1 — schema can stay v1.0 if the new classes fit the int16 column. |
