...
> **[STEP 0 — PRECONDITION, before ANY gate input is read; Umar-ruled 2026-06-10].** The eval-side `tile_dirname` calls (`geometry.py:89`, `holdout/pipeline.py:189`) default `epsg_label` to the Singapore `EPSG3414` — the SAME bug fixed in `build_shards_in_memory` at Task 8 (`ed1138c`), but on the EU **eval read** surface. This is NOT a loose carry-forward: gate input **(i)** is "a DATA property of REAL held-out tiles," read through exactly `geometry.py`'s path — so the diagnostic hits `FileNotFoundError` the moment it reads EU held-out (eisenhuttenstadt/glasgow/krakow/munich) tile data. **FIX THIS FIRST:** wire `epsg_label_for_region(region)` into the eval-side `tile_dirname` calls (same fix shape as Task 8); run the **three-way CRS consistency check** (region-config `projected_crs` == sub-D manifest `region_crs` == on-disk dir label) on the **4 held-out cities**; with its own **red-before/green-after on a real held-out read** (RED = `FileNotFoundError` on an EU held-out tile via the eval path; GREEN after the wiring). Only then measure any gate input. Recorded here so it is never discovered mid-diagnostic.
