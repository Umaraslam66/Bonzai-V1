...
**Behavior (spec Decisions 5+6):** iterate every tile dir (`tile=*` under the sub-F region); for each, load all four stages' artifacts via `readers`; run seam 1/2/3; **accumulate** all Diagnostics (no halt-on-first; spec Decision 6); aggregate seam-3 accuracy across tiles. After all tiles: `group_by_signature`; write `quarantine_report.yaml` + `_PHASE1_ACCURACY_BASELINE.yaml` (every run); apply the sanity floor (position p99.9 > 50m OR angle p95 > 20° → record a `sanity_floor_violated` group); write `_PHASE1_VALIDATED` iff `groups == [] AND not sanity_floor_violated`. The validator iterates **all** Singapore tiles (the §9 #2 all-Singapore gate-set) and records the tile count in its result, but the `<100`-tile scope-stop is enforced **PRE-CHAIN** (T11 Step-0b, before paying derive cost) — **not** at validator runtime. A region reaching T8 has already cleared the pre-chain gate, so T8 does not raise on tile count.

- [ ] **Step 1: Write the failing test** — synthetic region with 2 tiles: one all-passing (empty quarantine, marker written) and one with a seeded seam-1 mismatch (non-empty quarantine, NO marker, nonzero exit). Assert `quarantine_report.yaml` written in both cases (explicit `groups: []` when clean). (The `<100`-tile scope-stop is a pre-chain gate, tested at T11 Step-0b — not at validator runtime.)

- [ ] **Step 2: Run** → FAIL.

- [ ] **Step 3: Write minimal implementation** — `validate_region(region_root_dirs: dict, validator_version=VALIDATOR_VERSION) -> ValidationResult`. Real code wiring the readers + three seams + diagnostics + versions; the gate condition (`groups == [] AND not sanity_floor_violated`). Reuse `sub_d.io.read_macro_core_parquet` (pure byte-read) to get `zoning_class`/`cell_density_bucket`/`road_skeleton_class` keyed by CELL `(cell_i,cell_j)` and INTERNAL_EDGE `(lower_cell_i,lower_cell_j,axis)`.
...
- [ ] **Step 0a (cache precondition, before running):** confirm sub-C + sub-D Singapore caches exist on disk (`data/processed/sub_c/2026-04-15.0/singapore/_SUCCESS`, `…/sub_d/…/_SUCCESS`). If absent, STOP and surface to the reviewer — sub-C regeneration is an ~8-hour cold Overture fetch (out of sub-G scope).
- [ ] **Step 0b (tile-count scope gate — BEFORE any chain run):** count Singapore tiles via the sub-C `extract_tiles` output (or the sub-D `macro_core.parquet` CELL-slot row count). **If count < 100, STOP and surface to the reviewer per spec §9 #2 disposition — force re-approval before any chain run.** Do not pay the sub-E + sub-F derive cost (potentially hours) to discover a sub-100 region. (Singapore estimate 150–180 tiles, so this likely does not fire — but it is free to check, and this is the cost-saving location for the gate, not validator runtime.)
...
- **Open Step-0 reads carried into execution:** macro_plan_vocab structure (T2), zoning assignment rule (T4 SI-3 gate), `cell_edge_directions` (T5), canonicalize for angle/decomposition (T6), sub-C/sub-D cache presence + tile count (T11 Step-0a/0b). Each is a real executable step, not a placeholder.

---

## Execution discipline (esp. T7–T11 expansion)

T1–T6 are **verified-by-test** (failing test → real impl → passing test). When the implementer expands the `real code: …` directions in **T7–T11**, the behavior specs are precise but will not cover every micro-decision — e.g. the exact `volatile:` block YAML structure, chain-runner stdout-vs-file logging, or the CLI exit code (1 vs 2) for a `SubGScopeError`-class stop vs other failures. **Halt-on-judgment (sub-F posture):** when such an under-specified point surfaces, **surface it to the reviewer for ratification (or pre-write spec refinement) — do not pick a default.** T7–T11 expansion needs reviewer ratification on ambiguity points; T1–T6 expansion is self-verifying via its tests. The §"Design rules" provenance-citation / measure-from-source constraints remain non-negotiable throughout (especially: seam 2 never imports `load_boundary_contract`).
