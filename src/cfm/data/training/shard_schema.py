...
reads only `cells[*].tokens`, `cells[*].cell_density_bucket`, and
`tile_conditioning`. `macro_tokens` and `cells[*].boundary_contracts` are carried
so the bake-off's tile-AR (candidate 1) and hierarchical (candidate 2) runs read
the SAME shards — under-provisioning the write-once format to the slice's subset
is the fatal §10.1 direction.
