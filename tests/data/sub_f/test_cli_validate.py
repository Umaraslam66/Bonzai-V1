...
def valid_region(tmp_path):
    """A derived, valid 2-tile sub-F region. Returns (sub_f_out_dir, sub_e_dir, sub_c_dir).

    sub_c is returned because the v1.2 cross-tile validator reads it (the
    road-edge-presence signal for the symmetry + coverage legs); the CLI now
    takes --sub-c-region-dir.
    """
    sub_c, sub_d, sub_e, out = _build_region_inputs(tmp_path)
    derive_region(_make_cfg(sub_c, sub_d, sub_e, out))
    return out, sub_e, sub_c


def test_validate_happy_path_returns_zero(valid_region, capsys):
    out, sub_e, sub_c = valid_region
    rc = main(
        [
            "--region-dir",
            str(out),
...
def test_validate_missing_sub_e_arg_exits_2(valid_region):
    out, _sub_e, _sub_c = valid_region
    # --sub-e-region-dir (and, since v1.2, --sub-c-region-dir) are REQUIRED; the
    # one-arg snippet would have TypeError'd inside validate_cross_tile.
