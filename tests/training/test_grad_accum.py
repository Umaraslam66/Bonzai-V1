...
def test_effective_batch_constant_across_per_gpu_batch_via_grad_accum() -> None:
    ...
    # region is REQUIRED (no default); SG keeps this batch-arithmetic test region-agnostic
    small = ScaffoldConfig(region="singapore", batch_size=8, grad_accum=1, devices=4)
    large = ScaffoldConfig(region="singapore", batch_size=2, grad_accum=4, devices=4)
    assert effective_batch_size(small) == effective_batch_size(large) == 32


def test_grad_accum_defaults_to_one() -> None:
    assert ScaffoldConfig(region="singapore").grad_accum == 1
