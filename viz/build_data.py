...
def main() -> None:
    ...
    ap.add_argument(
        "--var",
        default="PROBE_DATA",
        help="global to assign (e.g. PROBE_DATA_MAMBA for the mamba bundle)",
    )
    args = ap.parse_args()

    bundle = build_probe_data(Path(args.inp))
    payload = json.dumps(bundle, separators=(",", ":"))
    src_dir = Path(args.inp).resolve().parent
    try:
        src_label = src_dir.relative_to(repo).as_posix()
    except ValueError:
        src_label = src_dir.name
    header = (
        f"// AUTO-GENERATED by viz/build_data.py from {src_label}/ "
        "(immutable). Do not edit by hand; re-run the builder.\n"
    )
    Path(args.out).write_text(header + f"window.{args.var} = " + payload + ";\n")
